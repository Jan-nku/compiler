...
public:
    enum {ADD, SUB, AND, OR, LESS, EQUAL, NOTEQUAL, LESSEQUAL, GREATER, GREATEREQUAL, MUL, DIV, MOD};
...
private:
    ExprNode* expr;
public:
    FunctionCall(SymbolEntry *se, ExprNode* expr = nullptr): ExprNode(se), expr(expr) {};
    void output(int level);
};
