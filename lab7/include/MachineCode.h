...
public:
    ...
    StackMInstrcuton(MachineBlock* p, int op, 
                std::vector<MachineOperand*> src, MachineOperand* src1, MachineOperand* src2=nullptr,
                int cond = MachineInstruction::NONE);
    void output();
};
...
public:
    ...
    void output();
    int getCmpCond() {return cmpcond;};
    void setCmpCond(int cond){cmpcond=cond;};
    int getSize(){return inst_list.size();};
};
...
public:
    ...
    * you can use this function in LinearScan::genSpillCode() */
    ...
    void output();
    std::vector<MachineOperand*> get_saved_regs();
...
private:
    ...
    int uniquenum=0;//就是n，应该是让addr_后面不一样，我加了个=0
public:
    std::vector<MachineFunction*>& getFuncs() {return func_list;};
    std::vector<SymbolEntry*>& getglobs() {return global_list;};
    ...
    void output();
    void insertGlobal(SymbolEntry* temp) {global_list.push_back(temp);};
    int getnum() {return uniquenum;};//对应getN
    //原来的那个printglobal被我内联到machinecode.cpp了
    int incnum() {int temp=uniquenum;uniquenum++;return temp;};
};
